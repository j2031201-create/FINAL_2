"""
슬기로운 영끌생활 · 부동산 투자 전략 게임 (캡스톤 최종판 v2.1)
시장원리 반영 · 영구해금 · 시장예측 · 실질수익 리포트 · sticky 스탯 · 설명서 팝업
[v2.1 패치] ① 변동 물가  ② NPC 확률적 의사결정  ③ 사전·사후 학습효과 측정 퀴즈
"""
...
APPRAISAL = {
    "수익환원법": {"icon":"💰","weapon":"⚔️ 수익환원법","easy":"월세를 보고 계산해요. \"이 건물 사면 몇 년 만에 본전 뽑을까?\"",
                  "def":"장래 임대수익(월세)을 환원율로 나누어 가치를 구하는 방식","best_for":["오피스텔","상가","빌라"],"best_txt":"오피스텔·상가"},
    "거래사례비교법":{"icon":"📊","weapon":"🏹 거래사례비교법","easy":"주변 거래 가격을 보고 계산해요. \"비슷한 건물이 얼마에 팔렸을까?\"",
                  "def":"인근의 실제 거래 사례를 보정하여 가치를 구하는 방식","best_for":["아파트"],"best_txt":"아파트"},
    "원가법":     {"icon":"🏗️","weapon":"🛡️ 원가법","easy":"다시 지으면 얼마 들지 계산해요. \"똑같은 건물 새로 지으면 얼마?\"",
...
INFLATION = 0.025         # [패치①] 기준(평균) 물가상승률 — 매 턴 이 값 주변에서 변동
INFLATION_MIN = 0.010     # [패치①] 연 물가 하한 (저물가)
INFLATION_MAX = 0.045     # [패치①] 연 물가 상한 (고물가)
TURN_MAX = 5
START_YEAR = 2026
...
def init():
    D={"phase":"intro","region":None,"capital":100000,"debt":0,"loan_rate":5.0,
       ...
       "news_popup":False,
       "confidence":60,
       "buy_count":0,"win_streak":0,"last_profit":None,
       "achievements":[],
       "new_achievements":[],
       "life_event":None,
       "apt_count":0,"rent_count":0,"max_debt":0,
...
def run_npc_turn():
    """[패치②] 룰 기반 + 확률적 변동 NPC 의사결정 (API 호출 없음).
    성향별 기본 전략을 따르되, 일정 확률로 차선책 선택·관망하여 예측 가능성을 낮춘다."""
    npc_market_update()
    ...
    for name, npc in S["npcs"].items():
        ...
        cands = list(S["market"])
        pick = None; comment = ""
        roll = random.random()  # [패치②] 성향별 흔들림 주사위

        if style=="공격형":
            apts = [L for L in cands if L["type"]=="아파트"]
            if roll < 0.20:
                # 20%: 변덕 — 이번엔 관망
                comment = "이번 장은 잠시 지켜보기로 했습니다."
            elif apts and (mood_good or npc["capital"]>0):
                # 80%: 평소 전략. 단, 가끔(30%) 최고가 아닌 차선 아파트로 분산
                apts_sorted = sorted(apts, key=lambda x:x["current"], reverse=True)
                if len(apts_sorted)>=2 and random.random()<0.30:
                    pick = apts_sorted[1]
                    comment = "최고가 대신 가성비 아파트로 갑니다."
                else:
                    pick = apts_sorted[0]
            ...
            # 기본은 호재 + 무대출일 때만. 가끔(15%) 약세장에도 소액 진입
            venture = roll < 0.15
            if (mood_good or venture) and affordable and npc["debt"]==0:
                pick = min(affordable, key=lambda x:x["current"])
                comment = "여유 자금으로 안전하게 한 채 담았습니다." if mood_good else "조정장이지만 싼 매물 하나만 담아봅니다."
            else:
            ...
            if roll < 0.20:
                comment = "조건이 더 좋은 매물을 기다리며 관망합니다."
            elif rentals:
                rentals_sorted = sorted(rentals, key=lambda x:x["monthly"], reverse=True)
                # 가끔(25%) 월세 2위(저평가 가능성) 선택
                if len(rentals_sorted)>=2 and random.random()<0.25:
                    pick = rentals_sorted[1]
                    comment = f"월세 2위지만 가격이 매력적이라 {pick['type']}를 담습니다."
                else:
                    pick = rentals_sorted[0]
...
def new_turn():
    ...
    turn_finance()
    # [패치①] 이번 해 물가율 결정: 기준 INFLATION 주변에서 변동(저물가~고물가),
    #          호재면 약간 낮게(안정), 악재면 약간 높게(불안정) 쏠리게 반영
    bias = -0.004 if S["news"]["mood"]=="good" else 0.004
...
.quiz-result{background:#1a2e4a;border:1.5px solid #9b59b6;border-radius:14px;padding:16px 18px;margin:10px 0;}
.qr-head{font-size:16px !important;font-weight:800;color:#c39bd3;margin-bottom:10px;}
.qr-score{font-size:15px !important;color:#e4eef8;line-height:1.7;}
.qr-badge{display:inline-block;font-weight:800;padding:3px 12px;border-radius:100px;font-size:14px !important;margin-top:6px;}
</style>
        ...
        for col,opt in zip(qc,q["opts"]):
            with col:
                if st.button(APPRAISAL[opt]["weapon"], key=f"qz_{opt}", use_container_width=True):
                    if opt==m:
                        # [패치③] 사전 학습수준 기록: 첫 시도(tries==0)에 맞췄는지
...
# ─────────────────────────────────────────────────────elif S["phase"]=="end":
    ...
    if S.get("inflation_history"):
        ...
        inflation_adj=int(start*(cum-1))
        infl_avg=sum(S["inflation_history"])/len(S["inflation_history"])
    else:
